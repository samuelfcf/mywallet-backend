...
describe('POST /log-in', () => {
  ...
  test('returns 400 for invalid body', async () => {
    const result = await supertest(app).post('/log-in').send({});
...
describe('GET /log-out', () => {
  beforeAll(async () => {
    await connection.query(
      "INSERT INTO sessions (user_id, token) VALUES (7, 'd6a21e3d-7933-4fba-b3ff-109d4fdcb035');"
    );
  });

  afterAll(async () => {
    await connection.query(
      "DELETE FROM sessions WHERE token='d6a21e3d-7933-4fba-b3ff-109d4fdcb035';"
    );
  });
